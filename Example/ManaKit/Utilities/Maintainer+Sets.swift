...
//        }
    ...
    private func updateSetSymbols() -> Promise<Void> {
        return Promise { seal in
            ...
            firstly {
                ...
                self.processKeyruneCodes(document: html)
            }.then { html in
                self.updateKeyruneCodes()
            }.done {
                seal.fulfill(())
            }.catch { error in
                seal.reject(error)
    ...
    private func processKeyruneCodes(document: HTMLDocument) -> Promise<Void> {
        return Promise { seal in
            try! realm.write {
    ...
    private func updateKeyruneCodes() -> Promise<Void> {
        return Promise { seal in
            try! realm.write {
