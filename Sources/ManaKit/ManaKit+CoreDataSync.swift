...
extension ManaKit {
    public func syncToCoreData<T: MEntity>(_ jsonData: [T],
        ...
        switch jsonType {
        case is MCard.Type:
            objectIDs = try await syncCards(jsonData: jsonData,
                                            jsonType: jsonType,
                                            to: context)
        case is MSet.Type:
            objectIDs = try await syncSets(jsonData: jsonData,
                                           jsonType: jsonType,
                                           to: context)
    ...
    private func syncSets<T: MEntity>(jsonData: [T],
                                      jsonType: T.Type,
    ...
    private func syncCards<T: MEntity>(jsonData: [T],
                                       jsonType: T.Type,
                                       to context: NSManagedObjectContext) async throws -> [NSManagedObjectID] {
        var newEntities = [T]()
    ...
    private func syncOtherData(of entity: MSet,
        ...
        if let cards = entity.cards {
            _ = try await syncCards(jsonData: cards,
                                    jsonType: MCard.self,
                                    to: context)
        }
    }
    ...
    private func syncOtherData(of entity: MCard,
        ...
        for componentPart in entity.componentParts ?? [] {
            _ = try await syncCards(jsonData: [componentPart.card],
                                    jsonType: MCard.self,
                                    to: context)
        ...
        for face in entity.faces ?? [] {
            _ = try await syncCards(jsonData: [face],
                                    jsonType: MCard.self,
                                    to: context)
        ...
        if let otherLanguages = entity.otherLanguages {
            _ = try await syncCards(jsonData: otherLanguages,
                                    jsonType: MCard.self,
                                    to: context)
        ...
        if let otherPrintings = entity.otherPrintings {
            _ = try await syncCards(jsonData: otherPrintings,
                                    jsonType: MCard.self,
                                    to: context)
        ...
        for variation in entity.variations ?? [] {
            _ = try await syncCards(jsonData: [variation],
                                    jsonType: MCard.self,
                                    to: context)
